...
void removevowels(char *str)
{
    ...
    for(i = 0; i < length; i++)
    {
        // Convert the current character to lowercase for uniform vowel checking
        char ch = tolower(str[i]);

        // Check if the character is NOT a vowel
        if(ch != 'a' && ch != 'e' && ch != 'i' && ch != 'o' && ch != 'u')
        {
            // If it's not a vowel, copy it to the position at index j
            // This effectively removes vowels by overwriting them
            str[j++] = str[i];
        }
    }

    // Null-terminate the resulting string to mark the end
    str[j] = '\0';
}
